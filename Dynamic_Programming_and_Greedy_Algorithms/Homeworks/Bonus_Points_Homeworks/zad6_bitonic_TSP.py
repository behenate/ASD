...
W problemie bitonicznego komiwojażera mamy daną listę miast:
C = [["Wrocław", 0, 2], ["Warszawa",4,3],
["Gdańsk", 2,4], ["Kraków",3,1]]
gdzie każde miasto jest opisane przez listę składających się z jego nazwy i współrzędnych x, y jego pozycji na mapie (dla uproszczenia pomijamy krzywiznę Ziemi). Miasta mają parami różne współrzędne x, ale mogą być podane w dowolnej
kolejności. Bitoniczny komiwojażer startuje w mieście o najmniejszej współrzędnej x i jego celem jest odwiedzić wszystkie miasta przy założeniu, że tylko raz
może zmienić kierunek z “w prawo” na “w lewo” (dokładny opis problemu przedstawiony jest na wykładzie).
Proszę zaimplementować funkcję verb—bitonicTSP( C )—, która wypisuje
na ekran długość trasy oraz po kolei miasta, które powinien odwiedzić komiwojażer. W przypadku miast z przykładu powyżej wynikiem może być:
